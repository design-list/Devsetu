...
export default function LibraryCards() {
  return (
    <section className="relative py-14 bg-gradient-to-b from-[var(--forcast)] via-white to-[var(--forcast)]">
      <div className="max-w-7xl mx-auto px-6 text-center relative z-10">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {categories.map((cat) => (
            <div
              key={cat.id}
              className="font-secondary group relative overflow-hidden rounded-2xl bg-white border border-gray-100 shadow-md hover:shadow-[0_10px_25px_rgba(0,0,0,0.1)] hover:-translate-y-1 transition-all duration-300"
            >
              {/* Image Wrapper */}
              <div className="relative w-full h-52 overflow-hidden">
                ...
                {/* Title on Image */}
                <h3 className="absolute bottom-3 left-4 text-white text-2xl font-semibold drop-shadow-lg">
                  {cat.title}
                </h3>
              </div>
              ...
              <div className="p-5 flex flex-col justify-between h-48">
                <p className="font-primary text-[var(--color-dark)] text-base leading-relaxed flex-grow">
                  {cat.desc}
                </p>
                <Link
                  href={cat.link}
                  className="font-primary inline-flex items-center justify-center gap-2 mt-4 text-white font-semibold bg-gradient-to-r from-[var(--color-primary-light)] to-[var(--color-primary)] rounded-lg py-2.5 hover:scale-105 hover:shadow-lg transition-all duration-300"
                >
      ...
      {/* Decorative Mandala Background (Optional) */}
