...
const PujaPackages = () => {
    ...
    const handleChange = async (e, index) => {
        ...
        if (files && files[0]) {
            ...
            setFormData((prev) => {
                const updated = [...(name === "packImg" ? prev.packages : prev.offerings)];
                updated[index][name] = localPreview;
                return {
                    ...prev,
                    [name === "packImg" ? "packages" : "offerings"]: updated,
                };
            });
            ...
            try {
                ...
                // ✅ Update server URL after upload success
                setFormData((prev) => {
                    const updated = [...(name === "packImg" ? prev.packages : prev.offerings)];
                    updated[index][name] = data.url; // replace preview with server URL
                    return {
                        ...prev,
                        [name === "packImg" ? "packages" : "offerings"]: updated,
                    };
