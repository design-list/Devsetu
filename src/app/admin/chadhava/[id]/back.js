...
import {
  fetchChadhavaDetailAction,
  updateChadhavaAction,
  requestChadhavaAction,
} from "@/redux/actions/chadhavaAction";

const UpdateChadhavaForm = () => {
  ...
  useEffect(() => {
    if (chadhavaDetail) {
      setFormData({
        ...chadhavaDetail,
        date: new Date(chadhavaDetail.date),
        banners: chadhavaDetail.banners || [{ imgUrl: "", type: "", position: 0 }],
        chadhavaFocus: chadhavaDetail.chadhavaFocus || [{ focusIcon: "", foucs: "" }],
        packages: chadhavaDetail.packages || [
          { packImg: "", title: "", description: "", price: "", currency: "INR", tags: "" },
        ],
        faqs: chadhavaDetail.faqs || [{ title: "", description: "" }],
      });
    }
  }, [chadhavaDetail]);
  ...
  const handleChange = async (e, index, section) => {
    ...
    if (files && files[0]) {
      ...
      // Local update before upload
      const updated = [...formData[section]];
      updated[index][name] = localPreview;
      setFormData({ ...formData, [section]: updated });

      try {
        const res = await fetch(`${baseAPIURL}/uploads`, { method: "POST", body: uploadFormData });
        const data = await res.json();
        if (res.ok) {
          updated[index][name] = data.storedAs;
          setFormData({ ...formData, [section]: updated });
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    fetchWithWait({ dispatch, action: updateChadhavaAction(formData) })
      .then((res) => {
        if (res.status === 200) {
  ...
  const handleToggle = (id, field, currentValue) => {
    ...
    Api.UpdeteChadhavaFlags(payload)
      .then((res) => {
        if (res.status === 200) dispatch(fetchChadhavaDetailAction(params.id));
        else alert(res.error);
      })
      .catch(console.error);
  };
...
export default UpdateChadhavaForm;
