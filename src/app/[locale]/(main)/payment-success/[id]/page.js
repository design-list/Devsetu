...
const PaymentSuccess = () => {
  ...
  return (
    <section className="min-h-screen bg-gradient-to-b from-[#fffaf5] via-[#fff6ee] to-[#fffaf2] py-16 px-6">
      <div className="max-w-6xl mx-auto bg-white/90 backdrop-blur-xl rounded-3xl shadow-[0_8px_30px_rgba(0,0,0,0.1)] border border-orange-100 p-8 md:p-12 transition-all duration-300 hover:shadow-[0_12px_40px_rgba(0,0,0,0.12)]">
        <BookingTracker
          ...
          >
            <div className="flex justify-between items-center flex-wrap gap-2">
              <span className="font-semibold text-orange-700 bg-orange-50 px-3 py-1.5 rounded-full shadow-sm">
                Booking ID: #{cartDetails.id}
              </span>
              {cartDetails?.["paymentStatus"] && (
                <span
                  className={`font-semibold px-3 py-1.5 rounded-full shadow-sm ${
                    cartDetails?.["paymentStatus"] === "success"
                      ? "bg-green-50 text-green-700"
                      : "bg-red-50 text-red-600"
                  }`}
                >
                  Payment: {cartDetails?.["paymentStatus"]}
                </span>
              )}
            </div>
          </motion.div>
