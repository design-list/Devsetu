...
const ArticleDetails = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const reqData = { category: "chalisa" };
    ...
    <main className="max-w-7xl mx-auto px-6 py-10">
      ...
      <nav className="text-sm text-gray-500 mb-6">
        <ol className="flex flex-wrap gap-2">
          ...
          <span>›</span>
          <li className="text-gray-700 font-medium">Chalisa Collection</li>
        </ol>
      </nav>

      {/* Page Title */}
      <h1 className="text-3xl font-bold mb-6">Chalisa Collection</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 ">
        ...
        {articles?.length === 0 ? (
          ...
          articles.map((item, index) => (
            <div
              key={index}
              onClick={() => handlaRedirectArticle("chalisa", item.slug)}
